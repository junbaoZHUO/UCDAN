...
class AlexNet(object):
    ...
    def create(self):
        ...
        gap5 = avg_pool(conv5, 13, 13, 1, 1, padding='VALID', name='gap5')
        s5 = conv(gap5, 1, 1, 16, 1, 1, padding='VALID', name='s5')
        e5 = conv_sig(s5, 1, 1, 256, 1, 1, padding='VALID', name='e5')
...
def conv_sig(x, filter_height, filter_width, num_filters, stride_y, stride_x, name,
         padding='SAME', groups=1):
    """Create a convolution layer.
    ...
    # Apply relu function
    sig = tf.nn.sigmoid(bias, name=scope.name)
...
def fc(x, num_in, num_out, name, relu=True):
    ...
    with tf.variable_scope(name) as scope:
        ...
        weights = tf.get_variable('weights', shape=[num_in, num_out],
                                  trainable=True, initializer=tf.random_normal_initializer(0, 0.001))
...
def avg_pool(x, filter_height, filter_width, stride_y, stride_x, name,
             padding='SAME'):
    """Create a max pooling layer."""
